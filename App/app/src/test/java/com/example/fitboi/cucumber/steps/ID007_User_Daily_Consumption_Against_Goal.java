package com.example.fitboi.cucumber.steps;

import io.cucumber.java.en.Given;
import io.cucumber.java.en.When;
import io.cucumber.java.en.Then;
import static org.junit.Assert.*;

public class ID007_User_Daily_Consumption_Against_Goal { 

    @When("viewing the screen")
    public void viewing_the_screen() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Then("the daily caloric consumtion values are displayed")
    public void the_daily_caloric_consumtion_values_are_displayed() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }

    @Then("the daily caloric consumtion values are not displayed")
    public void the_daily_caloric_consumtion_values_are_not_displayed() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @When("the connection between User and server is lost")
    public void the_connection_between_User_and_server_is_lost() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Then("no information is displayed")
    public void no_information_is_displayed() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
}