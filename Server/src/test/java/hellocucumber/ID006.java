package hellocucumber;

import io.cucumber.java.en.Given;
import io.cucumber.java.en.When;
import io.cucumber.java.en.Then;
import static org.junit.Assert.*;

public class ID006 { 
    @Given("the User is on the goals dashboard")
    public void the_User_is_on_the_goals_dashboard() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Given("there is no goal currently in progress")
    public void there_is_no_goal_currently_in_progress() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Given("the User has selected one of three preset goals")
    public void the_User_has_selected_one_of_three_preset_goals() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @When("clicking the set goal button")
    public void clicking_the_set_goal_button() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Then("the User adds a goal to their profile")
    public void the_User_adds_a_goal_to_their_profile() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Given("there is a goal currently in progress")
    public void there_is_a_goal_currently_in_progress() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Then("the User is given a prompt informing that they will override their current goal")
    public void the_User_is_given_a_prompt_informing_that_they_will_override_their_current_goal() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Then("the User clicks the confirm button")
    public void the_User_clicks_the_confirm_button() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Given("the User has entered a custom goal")
    public void the_User_has_entered_a_custom_goal() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Given("the data is invalid")
    public void the_data_is_invalid() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
    
    @Then("a User adds a goal to their profile")
    public void a_User_adds_a_goal_to_their_profile() {
        // Write code here that turns the phrase above into concrete actions
        throw new io.cucumber.java.PendingException();
    }
}